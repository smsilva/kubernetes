...
**Mudanças pendentes nesta sessão:**
- Fix nos scripts `02-install-haproxy-ingress`, `06-deploy-services`, `08-deploy-customer2` (pequenos ajustes)
- Fix no serviço `tenant-frontend`: correção no tratamento de URL e testes adicionais
...
- [ ] **Remover `COGNITO_CLIENT_ID` órfão dos ConfigMaps**: serviços não usam essa variável (vem do discovery via state JWT).
...
3. **Redirecionar para login quando token expirar** — atualmente Istio retorna 403 puro quando JWT expira. O serviço `tenant-frontend` deve detectar expiração (claim `exp`) e redirecionar para `/login` ao invés de mostrar erro. Alternativa: configurar Istio para redirecionar em vez de 403.
